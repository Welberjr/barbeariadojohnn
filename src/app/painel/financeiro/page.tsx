  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-2">
        <Link href={`/painel/financeiro?mes=${somarMeses(mes, -1)}`} aria-label="Mês anterior" className="btn-ghost p-3">
          <ChevronLeft className="w-5 h-5" />
        </Link>
        <p className="text-sm text-fg font-medium">{primeiraMaiuscula(mesLegivel(mes))}</p>
        <Link href={`/painel/financeiro?mes=${somarMeses(mes, 1)}`} aria-label="Próximo mês" className="btn-ghost p-3">
